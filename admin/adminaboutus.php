...
if ($num > 0) {
    ...
    for ($i = 0; $i < count($result); $i++) {
        ...
        echo <<<caselayout
         <img src="$imgpeople{$result[$i]['headimage']}"  alt=" "/>
         <div style="text-align: center">
         <p><a>{$result[$i]['name']}</a></p>
         <p><em>{$result[$i]['Profile']}</em></p>
         <p><input type="checkbox" name="employees[]" value="{$result[$i]['employeesId']}" />删除</p>
         </div>
     </div></div>
...
caselayout;
    ...
    echo '</div></div>' . "<div style=\"text-align:center;\" >全选/全不选<input type=\"checkbox\" name=\"all\" onclick=\"check_all(this,'employees[]')\" /></div>
      <br />
