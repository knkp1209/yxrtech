...
if ($num_cats > 0) {
    ...
    echo '<p style="text-align:center; font-size:1.2em">修改金牌案例轮播图，轮播图尺寸必须一致(建议尺寸 宽：1920px; 高：300-500px)，文件名不可以是中文或符号</p>';
    echo '<form method="post"
        action="updateslideshow.php?table=goldslideshows" enctype="multipart/form-data">';
    for ($i = 0; $i < count($result); $i++) {
        echo '<div id="oldimgdy' . $i . '" style="float: left;">';
        if (@file_exists($imggoldsilde . $result[$i]['image'])) {
            $size = GetImageSize($imggoldsilde . $result[$i]['image']);
            if (($size[0] > 0) && ($size[1] > 0)) {
                echo '<img id="vi' . $i . '"src="' . $imggoldsilde . $result[$i]['image'] . '" 
    ...
    echo '<p style="text-align:center; font-size:1.2em">修改金牌案例轮播图，轮播图尺寸必须一致(建议尺寸)，文件名不可以是中文或符号</p>';
    echo '<form method="post"
        action="updateslideshow.php?table=goldslideshows" enctype="multipart/form-data">';
    echo '<div id="picInput"><input type="file" name="imagefile[]" /></div>';
    echo '<input id="addBtn" type="button" onclick="addPic1()" value="继续添加图片" />';
